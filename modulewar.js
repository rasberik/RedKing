...
let WarManager = function(){
  ...
  this.upcoming = function (message, order) {
    ...
    var firstHalf = upcomingWars.map(war => "`" + (((war.order - startOrder) + (totalShow * order) + warCount) % warCount) + ".` " + getRewardInfo(war.reward).classemoji + "**`" + war.reward + "`** " + newline).reduce((prev, warInfo) => (prev) + warInfo)    
